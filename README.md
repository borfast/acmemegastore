...
tests I wrote should be enough for demonstration purposes.
...
means that each time Redis is restarted, all the cache will be wiped.
...
development. Instead, a file called `.env-dist` would be part of the project
just to specify which variable names should be set (no values in there) and 
for local development each developer could set their own values in a `.env`
file of their own, which could even be added to `.gitignore`.

### `requirements.txt` and the `requirements` folder

Typically small Python projects use a single `requirements.txt` file to keep
track of all their dependencies. For bigger (and even not so big) projects I 
prefer to separate the production requirements from the local development 
requirements - e.g. there's no reason to install the unittest package in 
production.
...
environments, production, local development, staging, etc.
...
tests a bit more realistic. These files are saved in the `fixtures` 
...
dependency creation occurs within other classes.
...
* I didn't write tests for absolutely everything.
